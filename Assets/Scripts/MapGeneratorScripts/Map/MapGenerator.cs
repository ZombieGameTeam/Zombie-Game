...
class MapGenerator{
	...
	public Vector2 getStartPosition() {
		List<Vector2> validStartingPoints = new List<Vector2>();
		
		if (this.initialPosition.x - 1 >= 0 && this.map[(int)this.initialPosition.x - 1, (int)this.initialPosition.y] == Cell.Block) {
			validStartingPoints.Add(new Vector2(this.initialPosition.x - 1, this.initialPosition.y));
		}
		if (this.initialPosition.x + 1 < map.size.x && this.map[(int)this.initialPosition.x + 1, (int)this.initialPosition.y] == Cell.Block)
		{
			validStartingPoints.Add(new Vector2(this.initialPosition.x + 1, this.initialPosition.y));
		}
		if (this.initialPosition.y - 1 >= 0 && this.map[(int)this.initialPosition.x, (int)this.initialPosition.y - 1] == Cell.Block)
		{
			validStartingPoints.Add(new Vector2(this.initialPosition.x, this.initialPosition.y - 1));
		}
		if (this.initialPosition.y + 1 < this.map.size.y && this.map[(int)this.initialPosition.x, (int)this.initialPosition.y + 1] == Cell.Block)
		{
			validStartingPoints.Add(new Vector2(this.initialPosition.x, this.initialPosition.y + 1));
		}
		
		int rand = Random.RandomRange(0, validStartingPoints.Count);
		
		return validStartingPoints[rand];
	}
